...
/*
  case 1: 
    ...
    timer.start();
    loadingTimer = millis(); 
    println ("loading timer " + loadingTimer); 
    //next = 1500; 
    println ("timer is " + timer); 
    image (loadingPage, 0, 0, width, height);
    textAlign (CENTER);
    //textSize (15); 
    ...
    text ("Your Genotypewriter is ready.", width/2, height/2 +50);
    ...
    if (timer.isFinished()) {
      if (frameCount % 100 < 50) {
        text ("collecting oxes ... ", width/2, height/2 + 65);
      }
      timer.start();
    } 
    else if (timer.isFinished()) {
      if (frameCount % 100 < 50) {
        text ("calibrating toothpicks ... ", width/2, height/2 + 80);
      }
      timer.start();
    } 
    else if (timer.isFinished()) {
      if (frameCount % 100 < 50) {
        text ("reticulating splines ... ", width/2, height/2 + 95);
      }
      timer.start();
    }
    else if (timer.isFinished()) {
  ...
  case 2:
    ...
    if (frameCount % 100 < 50) {
      text ("Hit The Lever To Continue", width/2, height-25);
    }
    break; 
*/
  case 1:  
